...
```bash
./run-tests.sh              # Run all 1266 tests
...
│   ├── test_kimi_session.py         # KimiAgentSession over ACP (experimental) (51 tests)
...
- `KimiAgentSession` over ACP: chunk aggregation, tool-call mapping, deferred tool input, commit-message + ask_user text protocols, resume/fallback, cancel, missing-SDK hint, permission decisions + handler, board-tools MCP config, errors (51)
